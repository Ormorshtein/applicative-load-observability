...
class TestCalcStress:
    ...
    def test_search_at_baseline(self):
        ...
        score, _ = calc_stress("_search", self._ctx())
        assert score == pytest.approx(1.0)

    def test_search_with_multiplier(self):
        score, _ = calc_stress("_search", self._ctx(), stress_multiplier=1.5)
    ...
    def test_search_zero_values(self):
        score, _ = calc_stress("_search", self._ctx(es_took_ms=0, hits=0, shards_total=0))
        assert score == pytest.approx(0.0)

    def test_bulk_at_baseline(self):
        score, _ = calc_stress("_bulk", self._ctx())
        expected = 0.45 * 1.0 + 0.55 * 1.0
        assert score == pytest.approx(expected)

    def test_bulk_ignores_multiplier(self):
        score_no, _ = calc_stress("_bulk", self._ctx())
        score_with, _ = calc_stress("_bulk", self._ctx(), stress_multiplier=2.0)
        assert score_no == score_with

    def test_update_by_query_at_baseline(self):
        score, _ = calc_stress("_update_by_query", self._ctx())
        expected = 0.40 * 1.0 + 0.35 * 1.0 + 0.25 * 1.0
        assert score == pytest.approx(expected)

    def test_delete_by_query_same_formula(self):
        ctx = self._ctx()
        score_del, _ = calc_stress("_delete_by_query", ctx)
        score_upd, _ = calc_stress("_update_by_query", ctx)
        assert score_del == score_upd

    def test_update_at_baseline(self):
        score, _ = calc_stress("_update", self._ctx())
        expected = 0.60 * 1.0 + 0.40 * 1.0
        assert score == pytest.approx(expected)

    def test_update_applies_multiplier(self):
        score, _ = calc_stress("_update", self._ctx(), stress_multiplier=1.5)
        expected = (0.60 * 1.0 + 0.40 * 1.0) * 1.5
        assert score == pytest.approx(expected)

    def test_create_at_baseline(self):
        score, _ = calc_stress("_create", self._ctx())
        expected = 0.70 * 1.0 + 0.30 * 1.0
        assert score == pytest.approx(expected)

    def test_create_ignores_multiplier(self):
        score_no, _ = calc_stress("_create", self._ctx())
        score_with, _ = calc_stress("_create", self._ctx(), stress_multiplier=2.0)
        assert score_no == score_with

    def test_index_at_baseline(self):
        score, _ = calc_stress("index", self._ctx())
        expected = 0.70 * 1.0 + 0.30 * 1.0
        assert score == pytest.approx(expected)

    def test_index_ignores_multiplier(self):
        score_no, _ = calc_stress("index", self._ctx())
        score_with, _ = calc_stress("index", self._ctx(), stress_multiplier=2.0)
        assert score_no == score_with

    def test_delete_at_baseline(self):
        score, _ = calc_stress("delete", self._ctx())
        expected = 0.70 * 1.0 + 0.30 * 1.0
        assert score == pytest.approx(expected)

    def test_delete_ignores_multiplier(self):
        score_no, _ = calc_stress("delete", self._ctx())
        score_with, _ = calc_stress("delete", self._ctx(), stress_multiplier=2.0)
        assert score_no == score_with

    def test_get_uses_doc_write(self):
        score, _ = calc_stress("get", self._ctx())
        expected = 0.70 * 1.0 + 0.30 * 1.0
        assert score == pytest.approx(expected)

    def test_get_applies_multiplier(self):
        score, _ = calc_stress("get", self._ctx(), stress_multiplier=1.5)
        expected = (0.70 * 1.0 + 0.30 * 1.0) * 1.5
        assert score == pytest.approx(expected)

    def test_unknown_operation_uses_doc_write(self):
        score, _ = calc_stress("_unknown", self._ctx())
        expected = 0.70 * 1.0 + 0.30 * 1.0
        assert score == pytest.approx(expected)
    ...
    def test_search_with_clause_bonus(self):
        ...
        ctx = self._ctx()
        score_no, _ = calc_stress("_search", ctx)
        score_with, bonuses = calc_stress("_search", ctx, clause_counts=_counts(bool_must_count=10))
    ...
    def test_clause_bonus_below_threshold(self):
        ...
        score_with, bonuses = calc_stress("_search", ctx, clause_counts=_counts(bool_must_count=3))
    ...
    def test_clause_bonus_capped(self):
        ...
        ctx = self._ctx()
        score_base, _ = calc_stress("_search", ctx)
        score_with, _ = calc_stress("_search", ctx, clause_counts=_counts(bool_must_count=1000))
    ...
    def test_clause_bonus_ignored_for_bulk(self):
        ...
        ctx = self._ctx()
        score_no, _ = calc_stress("_bulk", ctx)
        score_with, bonuses = calc_stress("_bulk", ctx, clause_counts=_counts(bool_must_count=20))
        assert score_no == score_with
        assert bonuses == {}
    ...
    def test_clause_bonus_multiplied_by_indicator(self):
        ...
        score, _ = calc_stress("_search", ctx, stress_multiplier=1.5, clause_counts=_counts(bool_must_count=10))
        assert score == pytest.approx(expected)
    ...
    def test_terms_values_bonus(self):
        ...
        ctx = self._ctx()
        score_base, _ = calc_stress("_search", ctx)
        score_with, bonuses = calc_stress("_search", ctx, clause_counts=_counts(terms_values_count=150))
    ...
    def test_at_threshold_no_bonus(self, key, at_threshold):
        ...
        ctx = self._ctx()
        score_base, _ = calc_stress("_search", ctx)
        score_with, _ = calc_stress("_search", ctx, clause_counts=_counts(**{key: at_threshold}))
        assert score_with == pytest.approx(score_base)
    ...
    def test_continuous_bonus_capped(self):
        ...
        ctx = self._ctx()
        score_base, _ = calc_stress("_search", ctx)
        score_with, _ = calc_stress("_search", ctx, clause_counts=_counts(agg_clause_count=10000))
        assert score_with == pytest.approx(score_base + 0.50)
    ...
    def test_continuous_bonuses_ignored_for_bulk(self):
        ...
        ctx = self._ctx()
        score_no, _ = calc_stress("_bulk", ctx)
        score_with, bonuses = calc_stress("_bulk", ctx, clause_counts=_counts(
    ...
    def test_multiple_bonuses_additive(self):
        ...
        score, bonuses = calc_stress("_search", ctx, stress_multiplier=1.5, clause_counts=_counts(
    ...
    def test_bonuses_dict_empty_when_no_counts(self):
        ...
        _, bonuses = calc_stress("_search", self._ctx())
        assert bonuses == {}
