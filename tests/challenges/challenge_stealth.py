...
def _print_summary(task_names, stats, stopped, guesses, found, start_time):
    ...
    if found:
        ...
        if n_guesses == 0:
            ...
            wrong = ", ".join(guesses)
            print(f"\n  Correct! '{CULPRIT}' was the silent killer.")
            print(f"  Found in {n_guesses + 1} guesses "
                  f"(wrong: {wrong})")
        print(f"\n  The culprit used zero exotic features — no scripts,")
        print(f"  no runtime mappings, no wildcards, no deep aggs.")
        print(f"  Just complex bool queries at moderate volume.")
    else:
        print(f"\n  The silent killer was: {CULPRIT}")
        print(f"  It triggered zero cost indicators — pure clause complexity.")
    print()
...
def main():
    parser = argparse.ArgumentParser(
        description="Challenge v3: find the silent killer")
