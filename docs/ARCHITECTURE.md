...
**Geo scoring uses vertex count, not area.** For `geo_shape` and `geo_polygon` queries, the stress bonus is based on total vertex count (tessellation CPU), not search area. ES tessellates query polygons into triangles at query time — the cost scales with vertices, not area. The area of a geo query mostly determines how many candidate documents match, which is already captured by the `response.hits` component of the stress formula. The vertex threshold is configurable via `STRESS_GEO_VERTEX_THRESHOLD` (default 10).

> **Future recommendation candidate:** Geo search area (`geo_area_km2`) and a `broad_geo` indicator were considered for scoring but removed — area is redundant with hits for scoring purposes. However, both are valuable as **recommendation signals** (e.g., "you're querying a 500km radius — is this intentional?"). When a recommendation engine is implemented, these should be revisited as stored fields with configurable thresholds.
