...
| Decision | Rationale |
|----------|-----------|
| Nginx/OpenResty as gateway | Battle-tested, C-speed proxying, no bottleneck risk. `workerProcesses` pinned to a fixed count (not `auto`) to prevent OOM in cgroup-limited containers where the host has many cores |
...
**Geo area uses bounding-box proxy.** For `geo_shape` and `geo_polygon` queries, ALO computes the search area from the bounding box of the polygon vertices rather than the true spherical polygon area. This is a deliberate simplification: the bounding box is trivial to compute, impossible to miscalculate, and conservative (always overestimates the area). It also matches how Elasticsearch optimizes geo queries — ES applies a bounding-box pre-filter before checking the actual shape, so the bbox area reflects the real initial scan cost. The tradeoff is that elongated shapes (e.g., a river buffer) are overscored because their bbox is much larger than their actual area. For stress scoring purposes, this is acceptable. For `geo_distance` queries, the exact circle area (π×r²) is used. The `broad_geo` cost indicator fires when the total geo area exceeds a configurable threshold (`COST_INDICATOR_GEO_AREA_KM2_THRESHOLD`, default 100 km²). The area bonus threshold is also configurable (`STRESS_GEO_AREA_BONUS_THRESHOLD_KM2`, default 1 km²).
...
│   └── Dockerfile                   # FROM openresty/openresty:alpine + lua-resty-http 0.17.2 + lua-resty-openssl
│
├── analyzer/
